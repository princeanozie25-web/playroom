...
| **C3** | **Dogwood-style temporal policy** | The evaluator gains a caller-computed **facts object** so authority can depend on history — kept pure, cost visible to the caller. **SHIPPED** (ADR-014): `evaluate` takes `facts`; a host grant's `requires: [tests_passed]` reads CONDITION_UNMET until the fact holds; facts assembled from `room_checks` (a node reports a check under its lease). The Drift rail "no push to main until tests pass" is real and tested. Self-reported check = the honest provenance limit. | A3 (event projection)                                         | **SHIPPED** (ADR-014)                                                                                                                                                                                     |

Acceptance test: **Drift cannot open a PR until tests pass; a revoked lease stops local work mid-flight.** (The Drift rail.) — **both halves now hold** (C2 revoke, C3 tests-gate).
