...
| S17-N1      | **`room-turns` is a `PartSource` but is MISSING from assembly's `allowed` and `order` arrays** (assembly.ts), so an order-rooted turn that carries prior completed turns throws its own §7.1 assertion and self-pauses. The loop the successor wires cannot run a briefed cycle until this is fixed. Pre-existing (S-LOOP); the S1.7 briefing was wired into ALL FOUR sites precisely to not repeat it                                                                                                         | **the loop-wiring slice** — add `room-turns` to both arrays (and a survives-pressure test), the same way `briefing` was added                   |
...
| S17-N3      | **rooms created before migration 026 have `created_by` = NULL**, so they are UN-BRIEFABLE (fail-closed: `briefing_no_room_owner`). New rooms get an owner; existing ones (demo-room, the film's room) cannot be briefed without a backfill or an owner-assignment path                                                                                                                                                                                                                                         | a backfill of `rooms.created_by`, or an owner-assignment surface, before an existing room needs a briefing                                      |
