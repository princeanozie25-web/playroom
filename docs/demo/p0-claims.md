...
| SL2-N1      | **PRODUCTION IS AT MIGRATION 024.** `025_raised_hand` and `026_room_briefings` are NOT applied to the `DATABASE_URL` database: `rooms.created_by` and `room_briefings` do not exist there, so neither S1.7's briefing nor SCC-3's raised hand exists on the live tier at all. Both Neon databases also hold **zero** `order.*` events and **zero** completed turns, so no loop has ever run against either                                                                                                                                                                                                                                                          | applying 025+026 to production — a WRITE to prod, which needs Prince's explicit go (see SCC3-N1); until then any "live" loop claim is unmeetable                                                                                                                                                      |
...
| SL2-N6      | **A CONTROL ROOM IS NOT A CONTROLLED EXPERIMENT.** Measuring a briefing by diffing two live rooms charges it for whatever the trigger member happened to write, since that reply rides into the cycle's window as `room-turns`: raw deltas were +65 and +46 across two runs, carry-corrected to ≈40 and ≈31. S1.7's ~4-chars/token estimate sits at the bottom of that range — reading low, but not wrong the way one run made it look. **The first draft of this claims section asserted "understated by roughly a third" from n=1 and was corrected by n=2 before it shipped**                                                                                    | a real number needs the trigger's output held fixed (a scripted trigger with a real cycle member), or enough runs to average the carry out — before any cost claim about context size is made without the word "estimate"                                                                             |
| SL2-N4      | **A RAISED HAND REACHES NOBODY WHOSE ROOM IS CLOSED.** A hand renders in the room as an interrupt chip (asserted at source in `raised-hand.test.ts`: the chip never reads `about_kind`, so a bare hand is drawn by the branch that draws every other claim) — and that is the ONLY surface it has. There is no push, no mail, no SMS: the loop's way of asking for a person is a chip on a page nobody is looking at. So a loop that stops silently at 3am stays stopped until Prince opens the room. **Not built in this slice, deliberately** — the attendance dial is what keeps the loop safe, and safety was never the notification's job                      | **HARD GATE: `max_unattended_cycles` may not be set above 1 in any live room until a raised hand demonstrably reaches Prince with the room closed.** The dial at 1 means a person sees every cycle before the next; above 1 the loop is relying on a claim on attention that provably does not arrive |
...
| S17-N4      | **the 390px capture is OUTSTANDING (exit criterion UNMET).** No live or local recording this slice of a briefing being set, rendered, and used by a summoned reply. Cause: `PLAYROOM_CAPTURE_HOME` is unset (the harness is gated on it), the live tier is deploy-blocked, and "a reply that used the briefing" needs a real provider turn. The render is proven at SOURCE (`briefing.test.ts`) and delivery by the assembly/puller tests instead — but that is not the on-camera artifact the brief asked for                                                                                                                                                      | set `PLAYROOM_CAPTURE_HOME` + run the app (or reach the live tier) and capture with a real summon; label the artifact local if not live                                                                                                                                                               |
